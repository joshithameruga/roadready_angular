...
@Pipe({
  name: 'searchReservationById'
})
export class SearchReservationByIdPipe implements PipeTransform {
...
transform(items: any[], searchValue: number, columnName: string): any[] {
    ...
    return items.filter(item => {
      ...
      return columnName === 'reservationId' && typeof item[columnName] === 'number' && item[columnName] === searchValue;
    });
  }
